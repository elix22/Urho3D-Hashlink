...
#include "Scripts/Utilities/ActionManager.as"
#include "Scripts/Utilities/GameObject.as"

Sprite@ logoSprite;
Scene@ scene_ = null;
String fullScenePath_ = "";
...
bool LoadScene_(const String&in fileName)
{
    log.Info("LoadScene  " + fileName);
    ...
    File file(fullPathFileName, FILE_READ);
    if (!file.open)
    {
	...
	if(loaded == true)
	{
		 Array<Node@> cameraNodes = scene_.GetChildrenWithComponent("Camera",true);
	  
		 // if there is a camera in the scene
		 // the first one found will be the main camera.
		 if(cameraNodes.length > 0)
		 {
		   for(uint i = 0 ; i < cameraNodes.length ; i++)
		   {
		     if(cameraNodes[i].enabled == true)
			 {
			    cameraNode = cameraNodes[i];
				break;
			 }
		   }
		 }
